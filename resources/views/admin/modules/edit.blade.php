...
@section('content')
    <div class="p-6">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-[#033E8A]">Edit Modul</h1>
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi modul pembelajaran.</p>
        </div>

        <div class="bg-white rounded-2xl shadow-sm p-6">
            <form action="{{ route('admin.modules.update', $modul->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="judul_modul" value="{{ old('judul_modul', $modul->judul_modul) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" rows="3"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]">{{ old('deskripsi', $modul->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="tingkat_kesulitan"
                        ...
                        <option value="mudah" {{ $modul->tingkat_kesulitan === 'mudah' ? 'selected' : '' }}>Mudah</option>
                        <option value="sedang" {{ $modul->tingkat_kesulitan === 'sedang' ? 'selected' : '' }}>Sedang</option>
                        <option value="sulit" {{ $modul->tingkat_kesulitan === 'sulit' ? 'selected' : '' }}>Sulit</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="id_gaya_belajar"
                        ...
                        @foreach($gayaBelajar as $gaya)
                            <option value="{{ $gaya->id }}" {{ $modul->id_gaya_belajar == $gaya->id ? 'selected' : '' }}>
                                {{ $gaya->nama_gaya }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="kategori" value="{{ old('kategori', $modul->kategori) }}"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]"
                ...
                <div class="mb-6">
                    ...
                    @if($modul->thumbnail)
                        <img src="{{ Storage::url($modul->thumbnail) }}" class="w-32 h-20 object-cover rounded-lg mb-2">
                    @endif
                    <input type="file" name="thumbnail" accept=".jpg,.jpeg,.png"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]">
                    <p class="text-xs text-gray-400 mt-1">Kosongkan jika tidak ingin mengubah thumbnail.</p>
                    @error('thumbnail')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div id="konten-wrapper" class="space-y-4">
                        @foreach($modul->isiModul as $isi)
                            <div class="konten-item border border-gray-200 rounded-xl p-4 relative">
                                @if(!$loop->first)
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="tipe_konten[]" onchange="toggleInput(this)"
                                        ...
                                        <option value="teks" {{ $isi->tipe_konten === 'teks' ? 'selected' : '' }}>Teks</option>
                                        <option value="gambar" {{ $isi->tipe_konten === 'gambar' ? 'selected' : '' }}>Gambar
                                        </option>
                                        <option value="video" {{ $isi->tipe_konten === 'video' ? 'selected' : '' }}>Video</option>
                                    </select>
                                </div>
                                ...
                                <div class="input-konten">
                                    ...
                                    @if($isi->tipe_konten === 'teks')
                                        <textarea name="isi_konten[]" rows="3"
                                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30">{{ $isi->isi_konten }}</textarea>

                                    @elseif($isi->tipe_konten === 'gambar')
                                        <img src="{{ Storage::url($isi->isi_konten) }}"
                                            class="w-32 h-20 object-cover rounded-lg mb-2">
                                        <input type="file" name="isi_konten[]" accept=".jpg,.jpeg,.png"
                                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                                        <input type="hidden" name="isi_konten[]" value="{{ $isi->isi_konten }}">
                                        <p class="text-xs text-gray-400 mt-1">Kosongkan jika tidak ingin mengganti gambar.</p>

                                    @elseif($isi->tipe_konten === 'video')
                                        @php
                                            preg_match('/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/', $isi->isi_konten, $ytMatches);
                                            $embedUrl = isset($ytMatches[1]) ? 'https://www.youtube.com/embed/' . $ytMatches[1] : null;
                                        @endphp
                                        @if($embedUrl)
                                        ...
                                        @endif
                                        <input type="text" name="isi_konten[]" value="{{ $isi->isi_konten }}"
                                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30"
                                            placeholder="https://youtu.be/xxxxx">
                                        <input type="hidden" name="isi_konten_lama[]" value="{{ $isi->isi_konten }}">
                                        <p class="text-xs text-gray-400 mt-1">Ubah URL jika ingin mengganti video.</p>
                                    @endif
                                </div>
                            </div>
                        @endforeach
