...
@section('content')
    <div class="p-6">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-[#033E8A]">Edit Kuis</h1>
            <p class="text-sm text-gray-500 mt-1">Ubah data soal kuis.</p>
        </div>

        <div class="bg-white rounded-2xl shadow-sm p-6">
            <form action="{{ route('admin.quizzes.update', $kuis->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="id_modul"
                        ...
                        @foreach($modul as $m)
                            <option value="{{ $m->id }}" {{ $kuis->id_modul == $m->id ? 'selected' : '' }}>
                                {{ $m->judul_modul }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="pertanyaan" rows="3"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]"
                        placeholder="Masukkan pertanyaan">{{ old('pertanyaan', $kuis->pertanyaan) }}</textarea>
                    @error('pertanyaan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="jawaban_benar"
                        ...
                        @foreach(['A', 'B', 'C', 'D'] as $jawaban)
                            <option value="{{ $jawaban }}" {{ $kuis->jawaban_benar === $jawaban ? 'selected' : '' }}>
                                {{ $jawaban }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="poin" value="{{ old('poin', $kuis->poin) }}" min="1"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]">
                    @error('poin')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="urutan" value="{{ old('urutan', $kuis->urutan) }}" min="1"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]">
                    @error('urutan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
