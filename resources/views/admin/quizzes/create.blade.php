    ...
    <div class="p-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6">
            <form action="{{ route('admin.quizzes.store') }}" method="POST">
                ...
                {{-- Urutan --}}
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Urutan Soal</label>
                    <input type="number" name="urutan" value="{{ old('urutan', 1) }}" min="1"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#033E8A]/30 focus:border-[#033E8A]"
                        placeholder="Nomor urutan soal (1, 2, 3, ...)">
                    <p class="text-xs text-gray-400 mt-1">Tentukan urutan tampil soal ini dalam kuis.</p>
                    @error('urutan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
